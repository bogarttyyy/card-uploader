...
def build_csv_data(
    ...
    export_df = pd.DataFrame(transactions_to_export_rows(transactions, metadata))
...
def render_card_summary(
    card_summary_rows: list[CardSummary],
        ...
        [
            {
                ...
                "CSV": _build_csv_link(
                    row.card_number,
                    build_csv_data(
                        get_transactions_for_card(all_transactions, row.card_number),
                        metadata,
                    ),
                ),
            }
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "CSV": st.column_config.LinkColumn("CSV", display_text="CSV"),
        },
    )
